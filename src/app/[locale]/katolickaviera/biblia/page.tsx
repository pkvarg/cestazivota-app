...
import contentData from '@/content/katolickaviera-biblia.json';

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'katolickaviera.nav' });
  return { title: `${t('biblia')} - Katolícka viera - cestazivota.sk` };
}
...
export default async function BibliaPage() {
  ...
  return (
    <div className="font-lora verse-kv">
      <section className="hero-kv-biblia min-h-[70vh] flex items-center justify-center py-20 px-4">
